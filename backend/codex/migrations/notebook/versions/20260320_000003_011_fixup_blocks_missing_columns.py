...
def upgrade() -> None:
    ...
    existing_cols = {row[1] for row in conn.execute(sa.text("PRAGMA table_info(blocks)")).fetchall()}
    ...
    for col_name, col_def in required_columns:
        if col_name not in existing_cols:
            conn.execute(sa.text(f"ALTER TABLE blocks ADD COLUMN {col_name} {col_def}"))
    ...
    has_file_id = "file_id" in existing_cols or "file_id" in {
        row[1] for row in conn.execute(sa.text("PRAGMA table_info(blocks)")).fetchall()
    }
    ...
    if has_file_id:
        ...
        si_cols = {row[1] for row in conn.execute(sa.text("PRAGMA table_info(search_index)")).fetchall()}
        if "file_id" in si_cols:
            ...
            conn.execute(sa.text("ALTER TABLE search_index_new RENAME TO search_index"))
            try:
        ...
        if "file_id" in [row[1] for row in cols_info]:
            ...
            conn.execute(sa.text(f"CREATE TABLE blocks_new ({', '.join(col_defs)})"))
            conn.execute(sa.text(f"INSERT INTO blocks_new ({cols_csv}) SELECT {cols_csv} FROM blocks"))
            conn.execute(sa.text("DROP TABLE blocks"))
            conn.execute(sa.text("ALTER TABLE blocks_new RENAME TO blocks"))
            ...
            try:
                conn.execute(
                    sa.text("CREATE UNIQUE INDEX IF NOT EXISTS uq_blocks_notebook_path ON blocks (notebook_id, path)")
                )
            except Exception:
                pass
