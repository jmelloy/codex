...
def test_list_notebooks(test_client, temp_workspace_dir):
    """Test listing notebooks in a workspace using nested route."""
...
def test_list_notebooks_empty_workspace(test_client, temp_workspace_dir):
    """Test listing notebooks in an empty workspace using nested route."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
...
def test_create_notebook(test_client, temp_workspace_dir):
    """Test creating a new notebook using nested route."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
...
def test_create_notebook_without_description(test_client, temp_workspace_dir):
    """Test creating a notebook without description using nested route."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
...
def test_create_notebook_generates_slug_path(test_client, temp_workspace_dir):
    """Test that notebook path and slug are generated from name."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
    ...
    assert response.status_code == 200
    data = response.json()
    # Path and slug should be slugified (lowercase, no special characters)
    assert data["path"] == "my-special-notebook"
    assert data["slug"] == "my-special-notebook"


def test_create_notebook_handles_name_collision(test_client, temp_workspace_dir):
    """Test that duplicate notebook names get unique paths and slugs."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
    ...
    # Paths and slugs should be different
    assert path1 != path2
    assert slug1 != slug2
...
def test_get_notebook_by_id(test_client, temp_workspace_dir):
    """Test getting a specific notebook by ID or slug using nested route."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
    ...
    data = response.json()
    assert data["id"] == notebook_id
    assert data["slug"] == notebook_slug
    assert data["name"] == "Get By ID Test"


def test_get_nonexistent_notebook(test_client, temp_workspace_dir):
    """Test getting a notebook that doesn't exist using nested route."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]

    response = test_client.get(
        f"/api/v1/workspaces/{workspace_slug}/notebooks/99999",
        headers=headers,
    )
...
def test_get_notebook_from_other_workspace(test_client, temp_workspace_dir, cleanup_workspaces):
    ...
    workspace1_slug = workspace1["slug"]

    # Create notebook in workspace1
    create_response = test_client.post(
        f"/api/v1/workspaces/{workspace1_slug}/notebooks",
    ...
    workspace2_slug = workspace2["slug"]
    cleanup_workspaces(workspace2["path"])

    # Try to access notebook using workspace2 slug
    response = test_client.get(
        f"/api/v1/workspaces/{workspace2_slug}/notebooks/{notebook_slug}",
        headers=headers,
    )
    assert response.status_code == 404
...
def test_get_notebook_indexing_status(test_client, temp_workspace_dir):
    """Test getting notebook indexing status using nested route."""
    headers, _ = setup_test_user(test_client)
    workspace = setup_workspace(test_client, headers, temp_workspace_dir)
    workspace_slug = workspace["slug"]
    ...
    # Get indexing status
    response = test_client.get(
        f"/api/v1/workspaces/{workspace_slug}/notebooks/{notebook_slug}/indexing-status",
        headers=headers,
    )
    assert response.status_code == 200
    data = response.json()
...
def test_list_notebook_plugins(test_client, temp_workspace_dir):
    ...
    # List plugins (still uses old route for plugin management)
    response = test_client.get(
...
def test_notebook_requires_authentication(test_client):
    ...
    # Test old route
    response = test_client.get("/api/v1/notebooks/?workspace_id=1")
    assert response.status_code == 401

    # Test new nested routes
    response = test_client.get("/api/v1/workspaces/test-workspace/notebooks")
    assert response.status_code == 401

    response = test_client.post("/api/v1/workspaces/test-workspace/notebooks", json={"name": "Test"})
    assert response.status_code == 401

    response = test_client.get("/api/v1/workspaces/test-workspace/notebooks/test-notebook")
    assert response.status_code == 401
