...
describe("View Path Resolver", () => {
  ...
  describe("resolve", () => {
    ...
    it("should resolve with leading slash", async () => {
      const mockNotebooks = [{ id: 1, name: "Project" }]
      const mockFiles = [{ id: 10, path: "views/kanban.cdx", filename: "kanban.cdx" }]
    ...
    it("should return null when file not found", async () => {
      const mockNotebooks = [{ id: 1, name: "Project" }]
      const mockFiles = [{ id: 10, path: "views/gallery.cdx", filename: "gallery.cdx" }]
    ...
    it("should handle complete failure gracefully", async () => {
      ...
      vi.mocked(notebookService.list).mockRejectedValue(new Error("Failed to list notebooks"))
  ...
  describe("caching", () => {
    ...
    it("should cache per workspace", async () => {
      const mockNotebooks = [{ id: 1, name: "Project" }]
      const mockFiles1 = [{ id: 10, path: "views/kanban.cdx", filename: "kanban.cdx" }]
      const mockFiles2 = [{ id: 20, path: "views/kanban.cdx", filename: "kanban.cdx" }]
  ...
  describe("matchesPath", () => {
    ...
    it("should not match different paths", () => {
      ...
      expect(viewPathResolver.matchesPath(file, "views/kanban.cdx")).toBe(false)
    })
  ...
  describe("preload", () => {
    ...
    it("should handle preload errors gracefully", async () => {
      vi.mocked(notebookService.list).mockRejectedValue(new Error("Network error"))

      const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => {})

      await expect(viewPathResolver.preload(["views/kanban.cdx"], 1)).resolves.not.toThrow()
