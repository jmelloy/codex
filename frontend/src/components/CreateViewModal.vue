  ...
  >
    <form @submit.prevent="handleCreate">
      ...
      <div v-if="selectedType" class="space-y-4 mt-4">
        ...
        <div
          v-if="selectedType === 'kanban'"
          class="border border-border-light rounded-md p-4 bg-bg-hover"
        >
          ...
          <FormGroup label="Status Property" v-slot="{ inputId }">
            ...
            <p class="text-xs text-text-tertiary mt-1">
              Property name used to group cards into columns
            </p>
          </FormGroup>
        </div>

        <div
          v-if="selectedType === 'gallery'"
          class="border border-border-light rounded-md p-4 bg-bg-hover"
        >
...
function generateViewContent(): string {
    ...
    case "dashboard":
      frontmatter.config = {
        layout: [
          {
            type: "row",
            components: [{ type: "mini-view", span: 12, view: "task-list.cdx" }],
          },
        ],
      }
      break
  ...
  function serializeValue(value: any): string {
    if (typeof value === "string") {
      ...
      if (
        value.match(/^(true|false|null|yes|no|on|off|\d+)$/i) ||
        value.includes(":") ||
        value.includes("#") ||
        value.includes("[") ||
        value.includes("{")
  ...
  yamlLines.push(
    "This is a dynamic view. Edit the frontmatter above to customize the query and configuration."
  )
  yamlLines.push("")
...
function handleCreate() {
  ...
  const finalFilename = filename.value.endsWith(".cdx") ? filename.value : `${filename.value}.cdx`
